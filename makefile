...
MOUNTFLAGS := -v $(CURDIR)/project:/home/alpine/project
...
PORTFLAGS  := -p 8080:8080 -p 4200:4200 -p 3000:3000
...
test :
	docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) --entrypoint /bin/bash $(IMAGETAG) -ec 'npm --version; cordova --version --no-telemetry; ionic --version'

help :
	docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) $(IMAGETAG) '--help'

start :
	docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) $(IMAGETAG) 'start'

serve :
	docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) $(IMAGETAG) 'serve'
